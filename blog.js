    ...
    .then(data => {
      ...
      blogs.forEach(blog => {
        ...
        const blogCard = `
          <div class="blog_card">
            <div class="blog_img">
              <a href="blog-details.html?id=${blog.documentId}">
                <img src="http://localhost:1337${imageUrl}" alt="${title}">
              </a>
            </div>
            <div class="blog_content">
              <span class="blog_date">${date}</span>
              <h2><a href="blog-details.html?id=${blog.documentId}">${title}</a></h2>
              <p>${description}</p>
              <a href="blog-details.html?id=${blog.documentId}" class="read_more">Read More →</a>
            </div>
          </div>
        `;
